...
function Devpage() {
  ...
  useEffect(() => {
    const getPerkDefs = async () => {
      try {
        const response = await axios.get(
          import.meta.env.VITE_API_URL + "perkDefs",
    ...
    const getFiles = async () => {
      try {
        const response = await axios.get(
          import.meta.env.VITE_API_URL + "perkFileDirs",
          { withCredentials: true }
        );
  ...
  const updateFromDB = async (fuseResult: FuseResult) => {
    ...
    for (let i = 0; i < newFuse.perks.length; i++) {
      ...
      const res = await axios.get(
        import.meta.env.VITE_API_URL + "perkByName/" + perk.name
      );
...
const FuseFixer = ({ fuseResult, files }: FuseFixerProps) => {
  ...
  const saveToDB = async (perks: FinalPerk[]) => {
    const res = await axios.get(import.meta.env.VITE_API_URL + "perks");
    ...
    axios
      .post(import.meta.env.VITE_API_URL + "updatePerks", perksToUpdate, {
        withCredentials: true,
      })
      .then((res) => {
...
const ImgInfo = ({ path, onClick }: ImgInfoProps) => {
  ...
  return (
    ...
    >
      <img
        className="h-24"
        src={import.meta.env.VITE_API_URL + "perkimg/" + path}
      ></img>
    </div>
  );
};
    ...
    <div className="bg-gray-600 min-w-fit p-2 rounded-xl gap-2">
      <div className="flex place-items-center justify-center">
        <img
          ...
          src={
            perk.imgUrl
              ? import.meta.env.VITE_API_URL + "perkimg/" + perk.imgUrl
              : ""
          }
        ></img>
        {perk.imgUrl ? (
